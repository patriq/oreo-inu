...
class FireGiants (ctx: ScriptContext) : BaseSlayerTaskInfo(ctx)  {
    override fun monsterNames(): Array<String>  = arrayOf("Fire giant")

    override fun standingArea(): Area = KourendCatacombs.Location.FIRE_GIANTS_WEST.area
    ...
    override fun walk(executingTask: Task) {
        if (KourendCatacombs.outsideMainEntrance() || KourendCatacombs.isInsideDungeon()) {
            KourendCatacombs.walk(KourendCatacombs.Location.FIRE_GIANTS_WEST)
            return
        }
